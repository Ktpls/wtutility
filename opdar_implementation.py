...
def planetrack(
    ...
    Xbdc=m*Xbdcrange
    contours=cv.findContours(Xbdcrange.astype('uint8'),cv.RETR_EXTERNAL,cv.CHAIN_APPROX_NONE)[0]
    ...
    sqrdist=(((info[:,0:1]-np.repeat(posref.reshape([1,2]),contnum,axis=0))/searchrange)**2).sum(axis=1)
...
def cameramotion(
    ...
    mask,
    subsamplerate=0.2):
    m0small=cv.resize(m0,None,fx=subsamplerate,fy=subsamplerate,interpolation=cv.INTER_AREA)
    masksmall=cv.resize(mask,None,fx=subsamplerate,fy=subsamplerate,interpolation=cv.INTER_AREA)
    ...
    m1small=cv.resize(m1,None,fx=subsamplerate,fy=subsamplerate,interpolation=cv.INTER_AREA)
