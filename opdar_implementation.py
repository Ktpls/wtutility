...
def planetrack(
    ...
    Xbdc = m*Xbdcrange
    contours = cv.findContours(Xbdcrange.astype(
        'uint8'), cv.RETR_EXTERNAL, cv.CHAIN_APPROX_NONE)[0]
    ...
    sqrdist = (((info[:, 0:1]-np.repeat(posref.reshape([1, 2]),
               contnum, axis=0))/searchrange)**2).sum(axis=1)
    ...
    cv.drawContours(mcontourmax, contours, maxscorecontourid,
                    1, thickness=cv.FILLED)
    clustermax = mcontourmax*m

    # (posx, posy), wingspan, clustermax, pul, its score
    return info[maxscorecontourid, 0:2], info[maxscorecontourid, 2], clustermax, pul, info[maxscorecontourid, 3]
...
def cameramotion(
        ...
        mask,
        subsamplerate=0.2):
    m0small = cv.resize(m0, None, fx=subsamplerate,
                        fy=subsamplerate, interpolation=cv.INTER_AREA)
    masksmall = cv.resize(mask, None, fx=subsamplerate,
                          fy=subsamplerate, interpolation=cv.INTER_AREA)
    ...
    m1small = cv.resize(m1, None, fx=subsamplerate,
                        fy=subsamplerate, interpolation=cv.INTER_AREA)

    # Calculate optical flow (i.e. track feature points)
    curr_pts, status, err = cv.calcOpticalFlowPyrLK(
        m0small, m1small, prev_pts, None)
